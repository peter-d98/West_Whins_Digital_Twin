"""Phase 3 ablation: separate the effect of the comfort floor from the
effect of using MPC.

Four arms, all sharing the same DHW seeds, legionella schedule, and
hedged forecast assumptions:

    A: threshold(45)  — status-quo dumb controller, strict floor
    B: threshold(40)  — dumb controller with relaxed floor (lazy threshold)
    C: MPC(40)        — MPC with relaxed floor (currently reported "mpc")
    D: MPC(45)        — MPC with strict floor

Ablation deltas (in £ on Octopus Agile):
    (B − A)   : pure setpoint-relaxation effect on dumb controller
    (D − A)   : pure MPC effect, both at strict floor
    (C − B)   : pure MPC effect, both at relaxed floor
    (C − A)   : combined (currently reported headline number)

Outputs (under output/control/csv/, plots/):
  * ablation_kpis.csv          (per-slice, per-arm, per-seed)
  * ablation_kpis_summary.csv  (mean / P05 / P95 by slice × arm)
  * ablation_cost_bars.png     (mean £ Agile per arm, grouped by slice)
"""
...
# (arm_id, controller_kind, floor_C)
ARMS = [
...
def main():
    ...
    wind_full = load_wind_mapped(_ROOT / "data" / "external" / "FWP_Generation_2019.csv",
                                 df.index)
    ...
    rows_df.to_csv(_CSV / "ablation_kpis.csv", index=False)
    ...
    summary = (rows_df.groupby(["label", "arm"])
               ...
               .reset_index())
    summary.to_csv(_CSV / "ablation_kpis_summary.csv", index=False)

    print("\n=== Ablation summary (mean over seeds, £ on Agile) ===")
    print(summary.pivot(index="label", columns="arm",
                        values="cost_agile_mean").round(2).to_string())

    print("\n=== Ablation deltas (£ vs A_threshold45 baseline) ===")
    ...
    ax.set_title("Ablation: comfort-floor relaxation vs MPC effect")
    ax.legend(title="arm", fontsize=8, loc="upper left")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(_PLOTS / "ablation_cost_bars.png", dpi=130, bbox_inches="tight")
    print(f"\nAll outputs → {_OUT}")
