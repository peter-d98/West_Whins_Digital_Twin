"""Top-node comfort ablation under the **Findhorn site flex tariff**.

Mirrors ``run_phase3_ablation_topnode.py`` but:

* MPC optimises against the Findhorn flex tariff (site-specific
  generation-driven price signal in ``data/external/Findhorn_flex_tariff.csv``).
* Costs are reported on the flex tariff (and the day/night tariff is
  also kept as a reference column).
* Three arms are run (skipping D_mpc45) since the question is
  "MPC@40 on flex tariff vs the strict-floor baseline":
      - A_top45 : threshold(top<45)             -- baseline
      - B_top40 : threshold(top<40)             -- floor-relax reference
      - C_mpc40 : MPC, comfort_node=top, floor=40, optimises flex tariff

Outputs: ``output/control/topnode_flex/``.
"""
...
_OUT   = _ROOT / "output" / "control" / "topnode_flex"
_CSV   = _OUT / "csv"
_PLOTS = _OUT / "plots"
...
def make_controller(kind, floor, *, times, T_amb, illum, flex_slice,
    ...
    if kind == "mpc":
        ...
        return MPCController(
            ...
            price_p_per_kwh=flex_slice,        # MPC optimises flex tariff
...
def main():
    ...
    for label, t0, t1 in SLICES:
        ...
        print(f"  flex tariff: min {flex_slice.min():.2f}p  "
              f"max {flex_slice.max():.2f}p  mean {flex_slice.mean():.2f}p  "
              f"({(flex_slice == flex_slice.min()).mean()*100:.0f}% at min)")

        # Plan legionella against the flex tariff (cheapest-window-targeted).
        ...
        for seed in range(N_SEEDS):
            ...
            for arm_id, kind, floor in ARMS:
                ...
                cost_flex  = cost_gbp(e_a, flex_slice)  + cost_gbp(e_i, flex_slice)
                cost_agile = cost_gbp(e_a, agile_slice) + cost_gbp(e_i, agile_slice)
                cost_dn    = cost_gbp(e_a, dn_slice)    + cost_gbp(e_i, dn_slice)
            ...
            print(f"    seed {seed}: " + "  ".join(
                f"{r['arm']}=\u00a3{r['cost_flex_gbp']:.2f}({r['ashp_kwh']:.1f}kWh,fires {r['n_ashp_fires']})"
                for r in rows[-len(ARMS):]))

    rows_df = pd.DataFrame(rows)
    rows_df.to_csv(_CSV / "ablation_kpis_topnode_flex.csv", index=False)

    summary = (rows_df.groupby(["label", "arm"])
               .agg(ashp_kwh_mean=("ashp_kwh", "mean"),
                    cost_flex_mean=("cost_flex_gbp", "mean"),
                    cost_flex_p05=("cost_flex_gbp",
                                   lambda s: float(np.quantile(s, 0.05))),
                    cost_flex_p95=("cost_flex_gbp",
               ...
               .reset_index())
    summary.to_csv(_CSV / "ablation_kpis_topnode_flex_summary.csv", index=False)

    print("\n=== Top-node flex-tariff summary (mean over seeds, \u00a3 flex) ===")
    print(summary.pivot(index="label", columns="arm",
                        values="cost_flex_mean").round(2).to_string())

    print("\n=== Deltas (\u00a3 flex vs A_top45 baseline) ===")
    base = (summary.set_index(["label", "arm"])["cost_flex_mean"]
            .unstack("arm"))
    deltas = pd.DataFrame({
        "B-A (relax floor only)":  base["B_top40"] - base["A_top45"],
        "C-B (MPC on top of relax)": base["C_mpc40"] - base["B_top40"],
        "C-A (combined)":           base["C_mpc40"] - base["A_top45"],
    }).round(2)
    print(deltas.to_string())
    deltas.to_csv(_CSV / "ablation_deltas_topnode_flex.csv")

    fig, ax = plt.subplots(figsize=(8, 5))
    pivot = summary.pivot(index="label", columns="arm",
                          values="cost_flex_mean")
    arm_order = [a[0] for a in ARMS]
    pivot = pivot[arm_order]
    pivot.plot(kind="bar", ax=ax, edgecolor="black",
               color=["#4477aa", "#88ccee", "#ee6677"])
    ax.set_ylabel("Mean total electricity cost on flex tariff [\u00a3 / 14 d]")
    ax.set_title("Top-node comfort ablation \u2014 Findhorn flex tariff")
    ax.legend(title="arm", fontsize=8, loc="upper left")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(_PLOTS / "ablation_cost_bars_topnode_flex.png",
